/* bubblesort.c
 * BST sort implementation using sorttemplate.c as a base
 * Inserts all elements into a BST then dumps them back via inorder traversal
 * Includes freeing BST nodes after sort
 */
...
// bstInsert - inserts x into BST, duplicates go right
bNode *bstInsert(bNode *t, int x) {
    if (t == NULL) {
        bNode *n = malloc(sizeof(bNode));
        n->data = x; n->left = NULL; n->right = NULL;
        return n;
    }
    if (x < t->data) t->left = bstInsert(t->left, x);
    else              t->right = bstInsert(t->right, x);
    return t;
}

// bstFill - inorder traversal, dumps values into array
int bstFill(bNode *t, int *a, int i) {
    if (t == NULL) return i;
    i = bstFill(t->left, a, i);
    a[i++] = t->data;
    i = bstFill(t->right, a, i);
    return i;
}

// freeBst - frees all BST nodes postorder
...
// sortarray - BST sort
// inserts all elements into a BST then dumps them back inorder
