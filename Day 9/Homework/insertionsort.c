/* insertionsort.c
 * Insertion sort implementation using sorttemplate.c as a base
 * Sorts by taking each element and moving it left into the correct position
 * Best case O(n) on sorted data, worst case O(n2)
 */
...
// sortarray - insertion sort
// takes each element and moves it left until it is in the right spot
