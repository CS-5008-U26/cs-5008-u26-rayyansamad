/* bubblesort.c
 * Merge sort implementation using sorttemplate.c as a base
 * Recursively splits array in half then merges sorted halves back together
 * O(n log n) regardless of data order
 */
...
// merge
//   merges two sorted halves a[lo..mid] and a[mid+1..hi] back together
...
// mergeHelper
//   recursively splits a[lo..hi] in half then merges back
void mergeHelper(int *a, int lo, int hi) {
    if (lo >= hi) return;
    int mid = (lo + hi) / 2;
    mergeHelper(a, lo, mid);
    mergeHelper(a, mid + 1, hi);
    merge(a, lo, mid, hi);
}

// sortarray - merge sort
// recursively splits array in half and merges sorted halves back together
