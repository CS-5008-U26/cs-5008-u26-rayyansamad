/* bubblesort.c
 * Quicksort implementation using sorttemplate.c as a base
 * Picks last element as pivot, partitions, recurses on each half
 * Average O(n log n), worst case O(n2) on sorted data
 */
...
// qsHelper
//   sorts a[start..end], uses a[end] as pivot, partitions, then recurses
void qsHelper(int *a, int start, int end) {
    if (start >= end) return;
    int pivot = a[end];
    int sep = start - 1;
    for (int i = start; i < end; i++) {
        if (a[i] < pivot) {
            sep++;
            int tmp = a[i]; a[i] = a[sep]; a[sep] = tmp;
        }
    }
    int tmp = a[end]; a[end] = a[sep+1]; a[sep+1] = tmp;
    qsHelper(a, start, sep);
    qsHelper(a, sep + 2, end);
}

// sortarray - quicksort
// picks last element as pivot, partitions array, recurses on each half
